...
int main() {
    ...
    for (auto& cont : continents) {
        ...
        while (n--) {
            ...
            phi = to_rad(phi), lamb = to_rad(lamb);
    ...
    rep(i,0,M-1) {
        ...
        if (angles.size() == 0) { if (inside) inside_length += angle_p1p2; }
        else {
            sort(angles.begin(), angles.end());
            long double prev_angle = 0;
            for (long double& angle : angles) {
                if (inside) { inside_length += angle - prev_angle; }
                prev_angle = angle;
                inside = !inside;
            }
            if (inside) { inside_length += angle_p1p2 - prev_angle; }
        }
    }
    full_length *= R;
    inside_length *= R;
