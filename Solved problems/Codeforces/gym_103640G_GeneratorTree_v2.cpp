...
vector<vector<pair<ll,int>>> acc_forward, acc_backward;
vector<umap<int,int>> child2index;
...
pair<ll,int> dfs_down(int idx, int u, int p, int e) {
    int d = edges[idx][e].ff == p;
    auto& ans = memo[2 * e + d];
    assert (ans.ff == -1);
    auto& tree = trees[idx];
    auto& acc_for = acc_forward[idx];
    auto& acc_bck = acc_backward[idx];
    vector<pair<ll, int>> hashes;
    int n = tree[u].size();
    hashes.reserve(n);
    for (auto& x : tree[u]) {
        if (x.ff == p) continue;
        auto h = dfs_down(idx, x.ff, u, x.ss);
        hashes.push_back(h);
    }
    sort(hashes.begin(), hashes.end());
    acc_for.reserve(n); acc_bck.reserve(n);
    ll h = 0; int c = 0;
    rep(i,0,n) {
...
pair<ll,int> rooted_hash(int idx, int root) {
    vector<vector<ii>>& tree = trees[idx];
    vector<pair<ll, int>> hashes;
    hashes.reserve(tree[root].size());
    for (auto& x : tree[root]) {
        int d = edges[idx][x.ss].ff == root;
        auto h = memo[x.ss * 2 + d];
        assert (h.ff != -1);
        hashes.push_back(h);
    }
    sort(hashes.begin(), hashes.end());
