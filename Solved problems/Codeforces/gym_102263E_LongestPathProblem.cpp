...
ll middle_dist(int u, int v) {
    int p = lca.lca(u, v);
    int steps = lca.steps_dist(u,v,p);
    ll dist =  lca.weight_dist(u,v,p);
    int l = 0, r = steps+1;
    ll md;
    while (l < r) {
        int m = (l+r) >> 1;
        ...
        if (lca.D[u] >= m) {
            mu = lca.raise(u, m);
            md = lca.weight_dist(u, mu, mu);
        } else {
            mu = lca.raise(v, steps-m);
            md = lca.weight_dist(u, mu, p);
        }
        if (md >= dist - md) {
            r = m;
        } else {
